...
function _merge (dst, src, _profiles, _clone) { // NOSONAR
  const profiled = []
  for (let p in src) {
    // const pd = Object.getOwnPropertyDescriptor(src,p)
    // if (!pd || !pd.value) continue
    const v = src[p]
    if (_profiles && p[0] === '[') {
      if (_profiles.includes(p.slice(1,-1))) profiled.push (()=> _merge (dst, v, _profiles, _clone))
      // delete dst[p]
    }
...
function _add_vcap_services_to (env, vcaps={}) { //NOSONAR
  ...
  for (let service in env.requires) {
    ...
    // credentials in VCAP are always complete, so overwrite what was there
    if (credentials)  any = conf.credentials = credentials
  }
  return any
