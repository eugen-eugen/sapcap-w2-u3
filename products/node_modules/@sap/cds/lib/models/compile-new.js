...
const compile = module.exports = Object.assign ((csn) => ({
  ...
  to: {
    ...
    hdbcds: (csn,o) => _2hdbcds (cdsc.to.hdbcds (csn, _options(o))),
    hana: (csn,o) => compile.to.hdbcds (csn,o),
    hdi: (csn,o) => _2many (cdsc.to.hdi (csn, _options(o))),
...
function compile4odata (csn, _options) {
  ...
  // REVISIT: only for old Java runtime --> need's to be double-checked
  if (o.sql_mapping)  ocsn['@sql_mapping'] = o.sql_mapping
...
function compile2edm ( fn, csn, _options, callback = x=>x ) {
  ...
  Pleasse choose by adding one of... \n
  ...
  const ocsn = compile.for.odata (csn,o)  //> REVISIT: check w/ Hans-Joachim + Simon: ist das richtig so, bzgl. optimierung oder sollte man alle edmxe in einem rutsch abholen?
...
function _options4odata (options) {
  ...
  o.odataVersion = o.version || cds.env.odata.version
  return o
}
...
function _options (options) {
  ...
  o.betaMode = cds.env.cdsc.betaMode
  return o
}
...
else if (!global.it && process.env.NODE_ENV !== 'test') console.warn ('[cds] - using new compiler options (snapi)')